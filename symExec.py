    ...
    while (idx < length):
        ...
        if name.startswith("tag"):
            ...
            if name == instr_name or name == "INVALID" and instr_name == "ASSERTFAIL" or name == "KECCAK256" and instr_name == "SHA3" or name == "SELFDESTRUCT" and instr_name == "SUICIDE":
                ...
                raise RuntimeError("Source map error, unknown name({name}) or instr_name({instr_name})")
    return idx
...
def check_callstack_attack(disasm):
    ...
    TOD = 0
    for i in range(0,len(disasm)):
        instruction_line = disasm[i]
        if instruction_line[1]=='PUSH' and instruction_line[2]=='4' and instruction_line[3]=='919840ad' :
           TOD=1

    if TOD:
        log.info('\t  Transaction Ordering Dependence Efficient:  \tTrue')
    else:
        log.info('\t  Transaction Ordering Dependence Efficient:  \tFalse')

    # Tx.origin
    TX = 0
    for i in range(0,len(disasm)):
        instruction_line = disasm[i]
        if instruction_line[1]=='ORIGIN' :
           TX=1

    if TX:
        log.info('\t  Tx_Origin:  \t\t\t\t\tTrue')
    else:
        log.info('\t  Tx_Origin:  \t\t\t\t\tFalse')
    ...
    for i in range(0,len(disasm)):
        instruction_line = disasm[i]
        if instruction_line[1]=='PUSH' and instruction_line[2]=='4' and instruction_line[3]=='ffffffff':
            if disasm[i-4][1]=='PUSH' and disasm[i-4][2]=='4' :
                CR=1


    if CR:
        log.info('\t  External Contract Referencing:  \t\tTrue')
    else:
        log.info('\t  External Contract Referencing:  \t\tFalse')
